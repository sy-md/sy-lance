...
class InventoryManager:
    ...
    def loading_data():
        with open(file_path, "r") as my_data:
    ...
    def weekly_parts(tmp):
        json_struc = tmp  # list
        tmp = {"data" : []}

        try:
            print("tyring to open the weekly json")
            with open(week, "r") as new_data:
                ...
                with open(week, "w") as new_data:
                    json.dump(data,new_data,indent=2)
        except FileNotFoundError:
            print("no json found creating it")
            os.system(f"touch {week}")
            with open(week, "w") as new_data:
                json.dump(tmp,new_data,indent=2)

            

    def start():
        get_data_keys = InventoryManager.loading_data()
        print(type(get_data_keys))
        part = qst.checkbox(
                "select the parts you have used to be tracked foe the week",
                choices=get_data_keys
        ).ask()
        tmp_dic = []
        for x in part:
            amt = input("how many part did use? ")
            tmp_dic.append([x, int(amt)])
            print(tmp_dic)



        InventoryManager.weekly_parts(tmp_dic)
